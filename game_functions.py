...
def create_alien(ai_settings,screen,aliens,alien_number,row_number):
    ...
    alien_width = alien.rect.width #给外星人的宽度赋值 矩形宽度
    alien.x = alien_width + 2 * alien_width * alien_number  # 这里的alien_number从0依次增加到number_aliens_x 从而使得alien.x的值等距增加
    alien.rect.x = alien.x  # 真正起作用的对象矩形赋值
    alien.rect.y = alien.rect.height + 2 * alien.rect.height * row_number
    aliens.add(alien)  # 每个新创建的外星人都添加到aliens编组中
